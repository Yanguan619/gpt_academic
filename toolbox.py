...
from config import UPLOAD_DIR,LOG_DIR
"""
========================================================================
第一部分
函数插件输入输出接驳区
...
def ArgsGeneralWrapper(fun):
    ...
    def decorated(cookies, max_length, llm_model, txt, txt2, top_p, temperature, chatbot, history, system_prompt, plugin_advanced_arg, *args):
        ...
        yield from fun(txt_passon, llm_kwargs, plugin_kwargs, chatbot_with_cookie, history, system_prompt, *args)
    return decorated
...
def write_results_to_file(history, file_name=None):
    ...
    with open(f'{LOG_DIR}/{file_name}', 'w', encoding='utf8') as f:
        ...
        for i, content in enumerate(history):
            ...
            f.write('\n\n')
    res = '以上材料已经被写入' + os.path.abspath(f'{LOG_DIR}/{file_name}')
    print(res)
    return res
...
def promote_file_to_downloadzone(file, rename_file=None):
    ...
    new_path = os.path.join(LOG_DIR, rename_file)
...
def on_file_uploaded(files, chatbot, txt, txt2, checkboxes):
    ...
    os.makedirs(f'{UPLOAD_DIR}/{time_tag}', exist_ok=True)
    err_msg = ''
    for file in files:
        file_origin_name = os.path.basename(file.orig_name)
        shutil.copy(file.name, f'{UPLOAD_DIR}/{time_tag}/{file_origin_name}')
        err_msg += extract_archive(f'{UPLOAD_DIR}/{time_tag}/{file_origin_name}',
                                   dest_dir=f'{UPLOAD_DIR}/{time_tag}/{file_origin_name}.extract')
    moved_files = [fp for fp in glob.glob(f'{UPLOAD_DIR}/**/*', recursive=True)]
    if "底部输入区" in checkboxes:
        txt = ""
        txt2 = f'{UPLOAD_DIR}/{time_tag}'
    else:
        txt = f'{UPLOAD_DIR}/{time_tag}'
        txt2 = ""
    moved_files_str = '\t\n\n'.join(moved_files)
...
def on_report_generated(files, chatbot):
    ...
    report_files = find_recent_files(LOG_DIR)
...
def zip_result(folder):
    ...
    zip_folder(folder, LOG_DIR, f'{t}-result.zip')
