...
load_dotenv(find_dotenv())  # 读取系统中的环境变量
# [step 1]>> 例如： API_KEY = "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"
# 可同时填写多个API-KEY，用英文逗号分割
API_KEY = os.getenv("OPENAI_API_KEY")  # 秘钥需要在环境变量中的OPENAI_API_KEY中设置
...
if USE_PROXY:
    ...
    proxies = {
        #          [协议]://  [地址]  :[端口]
        "http": "192.168.1.106:7892",  # 再例如  "http":  "http://127.0.0.1:7890",
        "https": "192.168.1.106:7892",  # 再例如  "https": "http://127.0.0.1:7890",
    }
else:
    proxies = None
...
CHATBOT_HEIGHT = 600  # 对话窗的高度
CODE_HIGHLIGHT = True  # 代码高亮

# 窗口布局
LAYOUT = "LEFT-RIGHT"  # "LEFT-RIGHT"（左右布局） # "TOP-DOWN"（上下布局）
DARK_MODE = False  # "LEFT-RIGHT"（左右布局） # "TOP-DOWN"（上下布局）

TIMEOUT_SECONDS = 30  # 发送请求到OpenAI后，等待多久判定为超时
WEB_PORT = -1  # 网页的端口, -1代表随机端口
MAX_RETRY = 2  # 如果OpenAI不响应（网络卡顿、代理失败、KEY失效），重试的次数限制

# 自定义保存路径
SAVE_DIR = "./chat"
UPLOAD_DIR = os.path.join(SAVE_DIR, "private_upload")
LOG_DIR = os.path.join(SAVE_DIR, "log")
# 自定义文件夹
PROMPTS_PATH = os.path.join(SAVE_DIR, "prompt.json")
...
CONCURRENT_COUNT = 100  # 设置gradio的并行线程数（不需要修改）
ADD_WAIFU = False  # 加一个live2d装饰
