...
def 全项目切换英文(txt, llm_kwargs, plugin_kwargs, chatbot, history, sys_prompt, web_port):
    ...
    os.makedirs(f'{LOG_DIR}/generated_english_version', exist_ok=True)
    os.makedirs(f'{LOG_DIR}/generated_english_version/crazy_functions', exist_ok=True)
    file_manifest = [f for f in glob.glob('./*.py') if ('test_project' not in f) and ('chat' not in f)] + \
                    [f for f in glob.glob('./crazy_functions/*.py') if ('test_project' not in f) and ('chat' not in f)]
    ...
    for index, h in enumerate(handles):
        ...
        where_to_relocate = f'{LOG_DIR}/generated_english_version/{fp}'
