...
def eval_manim(code):
    ...
    with open(f'{LOG_DIR}/MyAnimation.py', 'w', encoding='utf8') as f:
        f.write(code)
    ...
    try: 
        subprocess.check_output([sys.executable, '-c', f"from {LOG_DIR}.MyAnimation import {class_name}; {class_name}().render()"])
        shutil.move('media/videos/1080p60/{class_name}.mp4', f'{LOG_DIR}/{class_name}-{gen_time_str()}.mp4')
        return f'{LOG_DIR}/{gen_time_str()}.mp4'
