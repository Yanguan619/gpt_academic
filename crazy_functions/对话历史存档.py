...
def write_chat_to_file(chatbot, history=None, file_name=None):
    ...
    with open(f'{LOG_DIR}/{file_name}', 'w', encoding='utf8') as f:
        ...
        for h in history:
            f.write("\n>>>" + h)
        f.write('</code>')
    res = '对话历史写入：' + os.path.abspath(f'{LOG_DIR}/{file_name}')
    print(res)
    return res
...
def 载入对话历史存档(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if not success:
        if txt == "": txt = '空空如也的输入栏'
        import glob
        local_history = "<br/>".join(["`"+hide_cwd(f)+f" ({gen_file_preview(f)})"+"`" for f in glob.glob(f'{LOG_DIR}/**/chatGPT对话历史*.html', recursive=True)])
...
def 删除所有本地对话历史记录(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    import glob, os
    local_history = "<br/>".join(["`"+hide_cwd(f)+"`" for f in glob.glob(f'{LOG_DIR}/**/chatGPT对话历史*.html', recursive=True)])
    for f in glob.glob(f'{LOG_DIR}/**/chatGPT对话历史*.html', recursive=True):
        os.remove(f)
