...
def split_audio_file(filename, split_duration=1000):
    ...
    import os
    os.makedirs(f'{LOG_DIR}/mp3/cut/', exist_ok=True)  # 创建存储切割音频的文件夹
    ...
    for i in range(len(split_points) - 1):
        ...
        split_audio.write_audiofile(f"{LOG_DIR}/mp3/cut/{filename[0]}_{i}.mp3")
        filelist.append(f"{LOG_DIR}/mp3/cut/{filename[0]}_{i}.mp3")
...
def AnalyAudio(parse_prompt, file_manifest, llm_kwargs, chatbot, history):
    ...
    os.makedirs(f'{LOG_DIR}/mp3/', exist_ok=True)
    for index, fp in enumerate(file_manifest):
        ...
        if ext not in [".mp3", ".wav", ".m4a", ".mpga"]:
            ...
            audio_clip.write_audiofile(f'{LOG_DIR}/mp3/output{index}.mp3')
            fp = f'{LOG_DIR}/mp3/output{index}.mp3'
        # 调用whisper模型音频转文字
