...
def download_arxiv_(url_pdf):
    ...
    #     print('返回缓存文件')
    #     return f'{LOG_DIR}/arxiv/'+title_str
    ...
    x = x.replace('?', '？')\
        ...
        .replace('  ', ' ')\
        .replace('  ', ' ')
    return f'{LOG_DIR}/arxiv/'+title_str, other_info
...
def 下载arxiv论文并翻译摘要(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    import shutil
    # 重置文件的创建时间
    shutil.copyfile(pdf_path, f'{LOG_DIR}/{os.path.basename(pdf_path)}'); os.remove(pdf_path)
